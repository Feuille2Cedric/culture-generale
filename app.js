...
let sortOrder = 'az';
try { const savedOrder=localStorage.getItem('curio-sort-order'); if(['az','za','recent'].includes(savedOrder))sortOrder=savedOrder; } catch {}
function ordered(items){
 return items.map((value,index)=>({value,index})).sort((a,b)=>{
   if(sortOrder==='recent')return (b.value.updated??b.index)-(a.value.updated??a.index);
   const compared=titleCollator.compare(a.value.title?.trim()||'Sans titre',b.value.title?.trim()||'Sans titre');
   return (sortOrder==='za'?-compared:compared)||a.index-b.index;
 }).map(item=>item.value);
}
...
async function start() {
  try {
    ...
    const recovered=recoverNotionCategories(library);
    ready=true;status('● Enregistré dans ce navigateur');render();if(recovered)changed();if(window.startCurioSync)window.startCurioSync();
...
function changed() { window.curioSync?.mark();dirty=true;status('Enregistrement…');clearTimeout(timer);timer=setTimeout(()=>save(),450); }
async function save() {
  clearTimeout(timer);
...
function pageMatches(p){return [p.title,p.author||'',categoryOf(p)?.title||'',formats[kindOf(p)].label,...p.blocks.map(b=>b.text),library.topics.find(t=>t.id===p.topic)?.title].join(' ').toLocaleLowerCase('fr').includes(query.toLocaleLowerCase('fr'));}
function render(){
  if(!ready)return;
  $('#sort-order').value=sortOrder;
  $('#sort-control').hidden=!query&&['editor','reader'].includes(mode);
  $('#topics').innerHTML=ordered(library.topics).map(t=>`<button class="nav topic-nav ${['topic','reader','editor'].includes(mode)&&topicId===t.id?'active':''}" data-topic="${esc(t.id)}"><span class="dot ${esc(t.color)}"></span>${esc(t.title)}</button>`).join('');
...
function renderTopic(){const t=topic();$('#view').innerHTML=`<div class="hero"><div><span class="eyebrow">MON CARNET DE CONNAISSANCES</span><h1>${esc(t.title)}</h1><p>${esc(t.description)}</p></div><button class="secondary" data-action="edit-topic">Modifier le sujet</button></div>${categoryToolbar(t)}${groupedPages(filteredTopicPages())}`;}
...
function navigate(next,id){query='';$('#search').value='';mode=next;if(next==='topic'){if(topicId!==id)categoryFilter='all';topicId=id;}if(['editor','reader'].includes(next)){pageId=id;topicId=page().topic;if(next==='reader'){page().visited=Date.now();changed();}}render();}
...
$('#sort-order').onchange=event=>{sortOrder=event.target.value;try{localStorage.setItem('curio-sort-order',sortOrder);}catch{}render();};
...
document.addEventListener('click',event=>{
 ...
 if(d.createKind||d.action==='new-page'){const kind=d.createKind||'article';const p={id:uid(),topic:topicId,category:categoriesOf(topic()).some(c=>c.id===categoryFilter)?categoryFilter:'',kind,author:'',title:'',blocks:[{id:uid(),type:kind==='quote'?'quote':'text',text:''}],updated:Date.now(),favorite:false};library.pages.push(p);changed();navigate('editor',p.id);$('#page-title').focus();return;}
 ...
 if(d.add){if(d.add==='image'){imageTarget={pageId,library};$('#image-input').click();return;}page().blocks.push({id:uid(),type:d.add,text:'',url:''});page().updated=Date.now();changed();render();$('#blocks').lastElementChild?.querySelector('textarea,input')?.focus();return;}
...
$('#view').addEventListener('change',event=>{const el=event.target;if(el.id==='page-kind'){page().kind=el.value;page().updated=Date.now();changed();render();}if(el.id==='page-topic'){page().topic=el.value;page().category='';categoryFilter='all';topicId=el.value;changed();render();}if(el.dataset.layout){page().blocks.find(b=>b.id===el.dataset.layout).layout=el.value;changed();render();}});
...
function validateBackup(data){
 ...
 for(const t of data.library.topics){if(typeof t.id!=='string'||topicIds.has(t.id)||typeof t.title!=='string'||typeof t.description!=='string'||!colors.includes(t.color))throw Error('Sujet invalide dans la sauvegarde.');if(t.categories!==undefined){if(!Array.isArray(t.categories))throw Error('Sous-catégories invalides.');const ids=new Set(),names=new Set();for(const c of t.categories){if(!c||typeof c.id!=='string'||ids.has(c.id)||typeof c.title!=='string'||!c.title.trim()||names.has(categoryKey(c.title)))throw Error('Sous-catégorie invalide.');ids.add(c.id);names.add(categoryKey(c.title));}}topicIds.add(t.id);}
 for(const p of data.library.pages){if(typeof p.id!=='string'||pageIds.has(p.id)||!topicIds.has(p.topic)||typeof p.title!=='string'||!Number.isFinite(p.updated)||!Array.isArray(p.blocks))throw Error('Page invalide dans la sauvegarde.');if(p.kind!==undefined&&!Object.hasOwn(formats,p.kind))throw Error('Format de connaissance invalide.');if(p.author!==undefined&&typeof p.author!=='string')throw Error('Auteur invalide.');if(p.category!==undefined&&(typeof p.category!=='string'||p.category&&!categoriesOf(data.library.topics.find(t=>t.id===p.topic)).some(c=>c.id===p.category)))throw Error('Classement invalide.');pageIds.add(p.id);const blocks=new Set();for(const b of p.blocks){if(typeof b.id!=='string'||blocks.has(b.id)||!['text','heading','quote','image','link'].includes(b.type)||typeof b.text!=='string')throw Error('Bloc invalide dans la sauvegarde.');blocks.add(b.id);if(b.type==='image'&&(typeof b.image!=='string'||!/^data:image\/(png|jpeg|gif|webp);base64,[A-Za-z0-9+/=]+$/.test(data.images[b.image]||'')||!['wide','narrow','left','right'].includes(b.layout)))throw Error('Image invalide ou manquante.');if(b.type==='link'&&typeof b.url!=='string')throw Error('Lien invalide.');}}
 return data;
}
$('#import').onclick=()=>{if(ready)$('#import-input').click();};
$('#import-input').onchange=async event=>{const file=event.target.files[0];event.target.value='';if(!file)return;try{if(file.size>200*1024*1024)throw Error('Sauvegarde trop volumineuse (200 Mo maximum).');const data=validateBackup(JSON.parse(await file.text()));if(data.supplement){await importSupplement(data);return;}if(!confirm('Ajouter les sujets et les pages de cette sauvegarde à ta bibliothèque ? Tes contenus actuels seront conservés.'))return;recoverNotionCategories(data.library);const mapping=new Map(),imageMapping=new Map(),categoryMapping=new Map();for(const t of data.library.topics){const id=uid();mapping.set(t.id,id);const catMap=new Map();for(const c of categoriesOf(t)){const newId=uid();catMap.set(c.id,newId);c.id=newId;}categoryMapping.set(t.id,catMap);t.id=id;}const tx=db.transaction('images','readwrite'),completed=done(tx);for(const p of data.library.pages)for(const b of p.blocks)if(b.type==='image'){if(!imageMapping.has(b.image)){const id=uid();imageMapping.set(b.image,id);tx.objectStore('images').put(data.images[b.image],id);}b.image=imageMapping.get(b.image);}await completed;for(const p of data.library.pages){p.id=uid();if(p.category)p.category=categoryMapping.get(p.topic).get(p.category);p.topic=mapping.get(p.topic);for(const b of p.blocks)b.id=uid();}library.topics.push(...data.library.topics);library.pages.push(...data.library.pages);changed();await save();navigate('home');notice(dirty?'Import chargé, mais non enregistré. Exporte une sauvegarde avant de fermer.':'Sauvegarde importée. Tes contenus précédents ont été conservés.');}catch(error){notice(error.message||'Impossible d’importer ce fichier.');}};
